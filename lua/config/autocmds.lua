			...
			callback = function(tbl)
				...
				vim.keymap.set(
					Keys.N, "<LEADER>sf", function()
						...
						if notify ~= nil then
							notify(
								"Sourced file '" .. vim.fn.expand("%") .. "'", "info",
...
create_source_binding(
	{"*.vim", "*.lua"},
	function() vim.cmd.source({args = {vim.fn.expand("%")}}) end,
	"SourceVimscriptLua"
)
create_source_binding(
	{"*/config/sxhkd/**"}, function()
		vim.fn.jobstart(
			{
				"sh",
				"-c",
...
create_source_binding(
	{"*/dotfiles/config/xmobar/**"}, function()
		vim.fn.jobstart({"sh", "-c", "\"killall -9 xmobar ; xmobar & disown\""})
	end, "SourceXmobar"
)
create_source_binding(
	{"*/dotfiles/tmux-config/**"},
	function() vim.fn.jobstart({"tmux", "source", vim.fn.expand("%")}) end,
	"SourceTmux"
)
create_source_binding(
	{"*.cpp"}, function() vim.cmd("!runcpp " .. vim.fn.expand("%")) end,
	"SourceCpp"
)
