...
if exists('g:did_load_filetypes')
		...
		if exists('s:lesspipe_cmd') && executable('file')
				augroup lesspipe
						autocmd!
						autocmd BufReadPost * if  empty(&l:buftype) && !did_filetype() && !&l:binary && filereadable(bufname('%')) &&      system('file --mime --brief ' . fnamemodify(resolve(expand('%')), ':p:S')) !~# '^text/' |    silent exe '%!' . s:lesspipe_cmd . ' ' . expand('%:S') |    setlocal filetype=text nomodifiable readonly |  endif
				augroup END
		endif
endif
