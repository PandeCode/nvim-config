...
function add_wrap_format_stop_block(filetypes, start, stop)
	...
	vim.api.nvim_create_autocmd(
		"Filetype", {
			...
			callback = function(tbl)
				vim.keymap.set(
					"v", "<SPACE>fo",
					"vnoremap <buffer> <SPACE>fo <esc>`>a" .. stop .. "<esc>`<i" .. start ..
						"<esc>", {buffer = tbl.buf}
				)
			end
		}
	...
	vim.api.nvim_create_autocmd(
		"Filetype", {
			...
			callback = function(tbl)
				vim.keymap.set(
					"n", "<SPACE>fo", "<esc>{o" .. start .. "<esc>}O" .. stop .. "<esc>",
					{buffer = tbl.buf}
				)
			end
		}
...
function add_top_format_stop_block(filetypes, top)
	...
	vim.api.nvim_create_autocmd(
		"Filetype", {
			...
			callback = function(tbl)
				vim.keymap.set(
					"v", "<SPACE>fo", "<esc>`<i" .. top .. "<esc>", {buffer = tbl.buf}
				)
			end
		}
	)
	...
	vim.api.nvim_create_autocmd(
		"Filetype", {
			...
			callback = function(tbl)
				vim.keymap.set(
					"n", "<SPACE>fo", "<esc>{o" .. top .. "<esc>", {buffer = tbl.buf}
				)
			end
		}
	)
...
for _, wrap_format_stop_block in ipairs(wrap_format_stop_blocks) do
	add_wrap_format_stop_block(
		wrap_format_stop_block[1], wrap_format_stop_block[2],
		wrap_format_stop_block[3]
	)
end
